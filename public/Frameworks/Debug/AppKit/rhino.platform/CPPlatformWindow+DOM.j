I;21;Foundation/CPObject.jI;22;Foundation/CPRunLoop.ji;9;CPEvent.ji;17;CPCompatibility.ji;18;CPDOMWindowLayer.ji;18;CPPlatformWindow.jc;33664;
...
},["void"]), new objj_method(sel_getUid("isVisible"), function $CPPlatformWindow__isVisible(self, _cmd)
{ with(self)
{
    return _DOMWindow !== NULL;
}
},["BOOL"]), new objj_method(sel_getUid("orderFront:"), function $CPPlatformWindow__orderFront_(self, _cmd, aSender)
{ with(self)
{
    if (_DOMWindow)
...
},["void","id"]), new objj_method(sel_getUid("keyEvent:"), function $CPPlatformWindow__keyEvent_(self, _cmd, aDOMEvent)
{ with(self)
{
    var event,
...
{ with(self)
{
    ...
    if (windowNumber)
        location = objj_msgSend(CPApp._windows[windowNumber], "convertBridgeToBase:", location);
    if (type === "mouseup")
    {
        if(_mouseIsDown)
        {
